...
//compunere prin valoare ... sau  compunere prin obiect.. 
class Joc {

	char* nume;
	Erou erou; //pentru ca am luat aceasta compunere  .... intotdeauna in cosntructorii din Joc... se va apela implicit cosnt fara pram din Erou sa ne aloce memorie pt eroul din joc 
...
public:
	...
	Joc() {
		this->nume = new char[strlen("na") + 1];
		strcpy(this->nume, "na");
	 // Nu zici nimic pt  erou; deaorece s a apela implicit ... cosntructyorul fara parametri ala din calsa  Erou  si ti a pus pe this erou valori default 
	}

	Joc(const char* numeJoc, Erou e ) {
		this->nume = new char[strlen(numeJoc) + 1];
		strcpy(this->nume,  numeJoc);
		this->erou = e;// s a apelat op = si ti la facut pe eroul din this ca pe eroul e cel dat ca param 
	}
	Joc(const Joc& sursa) {
		this->nume = new char[strlen(sursa.nume) + 1];
		strcpy(this->nume, sursa.nume);
		this->erou = sursa.erou; 
	}
