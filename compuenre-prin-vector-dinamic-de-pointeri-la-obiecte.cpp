...
//compunere prin    la vector  static  de valori/obiecte  
class Joc {
...
public:
	...
	Joc(const char* numeJoc, int nre, Erou* ero  ) {
		...
		this->nrEroi = nre;
		this->eroi = new Erou * [ nre];
		for (int i = 0; i < nre ; i++) {
			this->eroi[i] = new Erou(ero[i]);
		}
	...
	Joc(const Joc& sursa) {
		...
		this->eroi = new Erou * [sursa.nrEroi];
		for (int i = 0; i < sursa.nrEroi; i++) {
			this->eroi[i] = new Erou (* sursa.eroi[i]);
		}
	}
	...
	~Joc() {
		...
		//mai itnai stergi fiecare obiect din fiecare pointer 
		for (int i = 0; i < this->nrEroi; i++) {
			if( this->eroi[i])
			delete this->eroi[i];
		}
 
		//apoi stergi vectoruld e pointeri
